...
public class MathProblem extends GPProblem {
    ...
    public MathProblem(GPConfiguration a_conf) throws InvalidConfigurationException {
        super(a_conf);
    }
...
//                        new Pow(conf, CommandGene.FloatClass),
                        new Terminal(conf, CommandGene.FloatClass, 0.0d, 100.0d, true),
                        // ADF-relevant:
...
//            x[i] = new Float(f);
            float f = -2.0f + i * 0.25f;
            x[i] = f;
            y[i] = f * f * f * f - 2 * f * f * f + f * f - 0.0000002f * f + 1.0f;
            // y = 1x4 - 2x3 + 1x2 - 2E-07x + 1
        ...
        // ------------------------------------------------------------------------
        return GPGenotype.randomInitialGenotype(conf, types, argTypes, nodeSets, 40, true);
    }
